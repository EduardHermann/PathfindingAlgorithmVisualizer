			...
			if event.type == pygame.KEYDOWN:
				if start is not None and end is not None:
					...
					if event.key == pygame.K_a:
						...
						found = False
						if AStar(lambda: draw(window, GRID, ROWS, width), GRID, start, end):
							found = True
						else:
							found = False
						my_info = str(time.time() - time_start)[:6]
						ctypes.windll.user32.MessageBoxW(0, "Execution Time: " + my_info + " seconds\n" + "Path Found: " + str(found), "AStar Information Message", 64)
						...
						time_start = time.time()
						found = False
						if Dijkstra(lambda: draw(window, GRID, ROWS, width), GRID, start, end):
							found = True
						else:
							found = False
						my_info = str(time.time() - time_start)[:6]
						ctypes.windll.user32.MessageBoxW(0, "Execution Time: " + my_info + " seconds\n" + "Path Found: " + str(found), "Dijkstra Information Message", 64)
